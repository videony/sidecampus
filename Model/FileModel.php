...
class FileModel {
    ...
    public function create($id_adder, $tx_commentaire, $tx_extension, $tx_name, $id_folder, $size, $encoded) {
        $query = "INSERT INTO file(id_adder, dt_ajout, tx_commentaire, tx_extension, tx_name, id_folder, int_size, tx_encoded_name)"
    ...
    public function remove()
    {
        ...
        else
        {
            ...
            $targetFolder = getcwd().FilesController::$filesFolder;
            $data = $this->get();
    ...
    public function del($filters, $mode = self::AND_MODE, $equality = self::EQUAl)
    {
        ...
        {
            $model = new FileModel($file['id_file']);
            $model->remove();
        }
        return TRUE;
    }
    ...
    {
        $query = 'SELECT * FROM file ORDER BY tx_name';
