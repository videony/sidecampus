 ...
 * @author videony
 */
class FilesController implements BodyController{
    ...
    public function canAccess() {
        if(!isset($_SESSION[SessionController::PLATFORM_ID]))
            return FALSE;
        if(!UserPermissionModel::can($_SESSION[SessionController::ID_MEMBRE], 'CAN_SEE_FILES'))
            return FALSE;
        return TRUE;
    }
    public function canSeeFile($fileid) {
        $model = new FileModel($fileid);
        $data = $model->get();
        $model = new FolderModel($data['id_folder']);
        $data = $model->get();
        if($data['id_plateforme'] != $_SESSION[SessionController::PLATFORM_ID])
            return FALSE;
        if(!UserPermissionModel::can($_SESSION[SessionController::ID_MEMBRE], 'CAN_SEE_FILES'))
            return FALSE;
        return TRUE;
    ...
    public function canSeeFolder($folderid) {
        $model = new FolderModel($folderid);
        $data = $model->get();
        if($data['id_plateforme'] != $_SESSION[SessionController::PLATFORM_ID])
            return FALSE;
        if(!UserPermissionModel::can($_SESSION[SessionController::ID_MEMBRE], 'CAN_SEE_FILES'))
    ...
    public function getContent() {
        ...
        {
            $model = new FolderModel($_REQUEST['folder']);
            $data = $model->get();
             if($data['id_plateforme'] == $_SESSION[SessionController::PLATFORM_ID]
        ...
        $template = GenerateUtils::getTemplate('Files');
        ...
        if(!UserPermissionModel::can($_SESSION[SessionController::ID_MEMBRE], 'CAN_MOVE_OWN_FILES')
                && !UserPermissionModel::can($_SESSION[SessionController::ID_MEMBRE], 'CAN_MOVE_OTHERS_FILES'))
        {
            $movefiles = FALSE;
        }
        else
        ...
        if(!UserPermissionModel::can($_SESSION[SessionController::ID_MEMBRE], 'CAN_REMOVE_OWN_FILES')
                && !UserPermissionModel::can($_SESSION[SessionController::ID_MEMBRE], 'CAN_REMOVE_OTHERS_FILES')
                && !UserPermissionModel::can($_SESSION[SessionController::ID_MEMBRE], 'CAN_REMOVE_OTHERS_FOLDERS')
                && !UserPermissionModel::can($_SESSION[SessionController::ID_MEMBRE], 'CAN_REMOVE_OWN_FOLDERS'))
        {
            $removefiles = FALSE;
        }
        else
        ...
        $template = GenerateUtils::replaceStrings($template, array(
            ...
            '###SUB_COMMENTS_FORM###'   =>  '',
            'if_can_add_files_button'   => UserPermissionModel::can($_SESSION[SessionController::ID_MEMBRE], 'CAN_ADD_FILES'),
            'if_can_add_folder_button'  => UserPermissionModel::can($_SESSION[SessionController::ID_MEMBRE], 'CAN_ADD_FOLDER'),
            'IF_CAN_MOVE_FILES'         => $movefiles,
            'IF_CAN_REMOVE_FILES'         => $removefiles
        ));
    ...
    private function generateBrowserWindow($template)
    {
        ...
        $folder = $_SESSION[SessionController::CURRENT_FOLDER];
        ...
        {
            if($curfolder['id_adder'] == $_SESSION[SessionController::ID_PERSONNE])
            {
                $folders[$key]['if_rename_folder'] = UserPermissionModel::can($_SESSION[SessionController::ID_MEMBRE], 'CAN_RENAME_OWN_FILES');
            }
            else
            {
                $folders[$key]['if_rename_folder'] = UserPermissionModel::can($_SESSION[SessionController::ID_MEMBRE], 'CAN_RENAME_OTHERS_FILES');
            }
        ...
        foreach($files as $key=>$file)
        {
            ...
            $files[$key]['tx_login'] = $persondata['tx_login'];
            ...
            if($file['id_adder'] == $_SESSION[SessionController::ID_PERSONNE])
            {
                $files[$key]['if_rename'] = UserPermissionModel::can($_SESSION[SessionController::ID_MEMBRE], 'CAN_RENAME_OWN_FILES');
                $files[$key]['if_comment_edit'] = UserPermissionModel::can($_SESSION[SessionController::ID_MEMBRE], 'CAN_EDIT_OWN_FILES_COMMENTS');
            }
            else
            {
                $files[$key]['if_rename'] = UserPermissionModel::can($_SESSION[SessionController::ID_MEMBRE], 'CAN_RENAME_OTHERS_FILES');
                $files[$key]['if_comment_edit'] = UserPermissionModel::can($_SESSION[SessionController::ID_MEMBRE], 'CAN_EDIT_OTHERS_FILES_COMMENTS');
            }
        ...
        if (!empty($_FILES) && isset($_SESSION['id_user'])) {
                ...
                if($this->putFile($tempFile, $encoded, TRUE))
                {
                    ...
                    $existing = $model->getFileByFilter(array(
                       'id_folder'  =>  $_SESSION[SessionController::CURRENT_FOLDER],
                        'tx_name'   =>  $filename
                    ));
                    $message = "";
                    if(empty($existing))
                    {
                        $id = $model->create($_SESSION[SessionController::ID_PERSONNE],
                                $filename,
                                $extension, $filename, $_SESSION[SessionController::CURRENT_FOLDER],
                                $size, $encoded);
                        $message = "Nouveau fichier ajouté: ";
                    }
                    else
                    {
                    ...
                    NotificationModel::fireNotif(NotificationModel::NOTIF_ON_NEW_FILE,
                            $message.$this->getChemin().$filename,
                            'request.php?action=Files&folder='.$_SESSION[SessionController::CURRENT_FOLDER].'#'.$id);
                    return '{"OK": 1}';
                }
        }
    }
    ...
    private function handleExtractingUpload()
    {
        ...
        $this->systemFilesToDatabase('ziptemp/'.$filename.'_zipdir', $_SESSION[SessionController::CURRENT_FOLDER]);
    ...
    public function systemFilesToDatabase($dir, $parentdir)
    {
        ...
        {
            if(UserPermissionModel::can($_SESSION[SessionController::ID_MEMBRE], 'CAN_ADD_FOLDER'))
            {  
    ...
    private function encodeFilename($filename, $directory = NULL){
        if($directory == NULL)
            $directory = $_SESSION[SessionController::CURRENT_FOLDER];
        // Implémentation
        ...
        $encoded = sha1($_SESSION[SessionController::PLATFORM_ID]
                    .$directory
                    .$filename);
        do
        {
    ...
    public function changeFolder($id){
        ...
        foreach($available as $folder)
        {
            ...
            {
                $_SESSION[SessionController::CURRENT_FOLDER] = $id;
                return TRUE;
            }
        }
    ...
    public function getChemin() {
        $model = new FolderModel($_SESSION[SessionController::CURRENT_FOLDER]);
        $data = $model->get();
        $chemin = '';
        while($data['id_parent_folder'] != NULL)
        {
            $chemin = $data['tx_name'].'/'.$chemin;
            $model = new FolderModel($data['id_parent_folder']);
            $data = $model->get();
        }
        $chemin = '/'.$chemin;
        return $chemin;
        ...
        switch($_POST['action'])
        {
            case 'add_folder':
                if(!UserPermissionModel::can($_SESSION[SessionController::ID_MEMBRE], 'CAN_ADD_FOLDER'))
                    return 'NO';
                $model = new FolderModel();
                if(!$model->folderExists($_SESSION[SessionController::CURRENT_FOLDER], $_POST['name']))
                {
                    $return = $model->create($_POST['name'], $_SESSION[SessionController::ID_PERSONNE],
                            $_SESSION[SessionController::CURRENT_FOLDER],
                            $_SESSION[SessionController::PLATFORM_ID]);
                }
                else
                {
                ...
                NotificationModel::fireNotif(NotificationModel::NOTIF_ON_NEW_FOLDER,
                        'Nouveau dossier: '.$this->getChemin().$_POST['name'],
                            'request.php?action=Files&folder='.$_SESSION[SessionController::CURRENT_FOLDER].'#'.$return);
            ...
            case 'change_folder_by_path':
                ...
                $cur = $model->getDefaultFolder($_SESSION[SessionController::PLATFORM_ID]);
                $cur = $cur['id_folder'];
            ...
            case 'comments_post_upload':
                if(!UserPermissionModel::can($_SESSION[SessionController::ID_MEMBRE], 'CAN_ADD_FILES'))
                    return 'NO';
                if(!isset($_SESSION['new_files']))
                {
                    return '';
                }
                $html = GenerateUtils::getTemplate('Files');
            ...
            case 'delete_selection':
                ...
                foreach($_POST['selection'] as $elem)
                {
                    ...
                    if($type == 'file'){
                        $model = new FileModel($id);
                        $data = $model->get();
                        $notif = NotificationModel::NOTIF_ON_DELETE_FILE;
                        $notifText = "Fichier supprimé: ".$this->getChemin().$data['tx_name'];
                    }
                    elseif($type == 'folder'){
                        $model = new FolderModel($id);
                        $data = $model->get();
                        $notif = NotificationModel::NOTIF_ON_DELETE_FOLDER;
                        $notifText = "Dossier supprimé: ".$this->getChemin().$data['tx_name'];
                    }
                    ...
                    if($data['id_adder'] == $_SESSION[SessionController::ID_PERSONNE])
                    {
                        if(!UserPermissionModel::can($_SESSION[SessionController::ID_MEMBRE], 'CAN_REMOVE_OWN_FILES'))
                            ...
                            NotificationModel::fireNotif($notif, $notifText);
                            $model->remove();
                        }
                    }
                    else
                    {
                        if(!UserPermissionModel::can($_SESSION[SessionController::ID_MEMBRE], 'CAN_REMOVE_OTHERS_FILES'))
                            $notall = TRUE;
                        else{
                            NotificationModel::fireNotif($notif, $notifText);
                            $model->remove();
            ...
            case 'paste_selection':
                ...
                break;
            case 'edit_comment':
                $model = new FileModel($_POST['id']);
                $data = $model->get();
                // Permissions
                if($data['id_adder'] == $_SESSION[SessionController::ID_PERSONNE])
                {
                    if(!UserPermissionModel::can($_SESSION[SessionController::ID_MEMBRE], 'CAN_EDIT_OWN_FILES_COMMENTS'))
                        return 'NO';
                }
                else
                {
                    if(!UserPermissionModel::can($_SESSION[SessionController::ID_MEMBRE], 'CAN_EDIT_OTHERS_FILES_COMMENTS'))
                        return 'NO';
                }
                ...
                break;
            case 'rename_folder':
                $model = new FolderModel($_POST['id']);
                $data = $model->get();
                // Permissions
                if($data['id_adder'] == $_SESSION[SessionController::ID_PERSONNE])
                {
                    if(!UserPermissionModel::can($_SESSION[SessionController::ID_MEMBRE], 'CAN_RENAME_OWN_FILES'))
                        return 'NO';
                }
                else
                {
                    if(!UserPermissionModel::can($_SESSION[SessionController::ID_MEMBRE], 'CAN_RENAME_OTHERS_FILES'))
                        return 'NO';
                }
                return $model->set(array(
                   'tx_name' =>  $_POST['name']
            ...
            case 'change_nb_per_line':
                ...
                break;
            case 'upload_limit':
                return MAX_MB_UPLOAD_SIZE;
                break;
            case 'authorize_view':
                if(isset($_REQUEST['id_file']))
                {
                    $model = new FileModel($_REQUEST['id_file']);
                    if($this->canSeeFile($_REQUEST['id_file']))
                    {
                        ...
                        switch(strtolower($data['tx_extension']))
                        {
                            ...
                            case 'pbm':
                            case 'pct':
                                return 'dim.php?type=file_img_type&file='.$_REQUEST['id_file'].'&timestamp='.time();
                                break;
                            ...
                            case 'xspf':
                            case 'yml':
                                return 'txtview.php?action=see_file&key='.$_REQUEST['id_file'];
                        break;
                            default:
                                $key = '';
                                ...
                                FileViewAuthorizationModel::PLATFORM_FILE);
