function cut() {
    var selection = new Array();
            ...
            var html = sendAsyncPostRequest({
                  handler: 'PersonalFilesController',
...
//------------------------------------------------------------------------------- 
   ...
   if($('#add_file_form').length > 0)
   {
       ...
       var uploader = new plupload.Uploader({
            ...
            url: 'request.php?action=PersonalFiles&upload_files=1',
          ...
          uploader.bind('UploadComplete', function(up, err) {
              uploader.setOption('url', 'request.php?action=PersonalFiles&upload_files=1&timestamp='+Date.now());
              $('#filelist').html('');
...
//-------------------------------------------------------------------------------    
      ...
      $(document).on('click', '.download_file', function(){
          ...
          if(size > max_download_size)
          {
           ...
           var lien = 'pdfi.php?action=download_file&id='+$(this).closest('div.fileline').attr('id')+'&timestamp='+(new Date().getTime());
           $.fileDownload(lien, {
      ...
      $(document).on('click', '.download_folder', function(){
          ...
          if(size > max_download_size)
          {
              var message = 'Vous ne pouvez pas télécharger plus de '+max_download_size+' MO à la fois. ';
              alert(message);
              return false;
          }
           var $preparingFileModal = $("#preparing-file-modal");
           $preparingFileModal.dialog({ modal: true });
           var lien = 'pdfi.php?action=download_folder&id='+$(this).closest('div.fileline').attr('id')+'&timestamp='+(new Date().getTime());
           $.fileDownload(lien, {
               successCallback: function (url) {
                   $preparingFileModal.dialog('close');
               },
      ...
      $(document).on('click', '.download', function(){
          if($('#selection_size').text() > max_download_size)
          {
              var message = 'Vous ne pouvez pas télécharger plus de '+max_download_size+' MO à la fois. ';
              alert(message);
              return;
          ...
          if($('.selected').length == 0)
           ...
           /*var $preparingFileModal = $("#preparing-file-modal");
           $preparingFileModal.dialog({ modal: true });*/
           ajaxindicatorstart('Acquisition du fichier en cours...');
           var lien = 'pdfi.php?action=download_selection&timestamp='+(new Date().getTime());
           $.fileDownload(lien, {
               successCallback: function (url) {
                   ajaxindicatorstop();  
               },
