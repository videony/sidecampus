...
class Mail {
    ...
    public function setHeader ($header, $value) {
        ...
        switch ($header) {
            ...
            case self::HEADER_CONTENT_TRANSFERT_ENCODING:
                if (!in_array($value, array('7bits', '8bits', 'binary', 'quoted-printable', 'base64', 'ietf-token', 'x-token', 'utf-8')))
    ...
    public function send () {
        ...
        if(!isset($this->_headers[self::HEADER_CONTENT_TYPE]))
            $this->_headers[self::HEADER_CONTENT_TYPE] = "text-html";
        if(!isset($this->_headers[self::HEADER_CONTENT_TRANSFERT_ENCODING]))
            $this->_headers[self::HEADER_CONTENT_TRANSFERT_ENCODING] = "utf-8";

        foreach ($this->_headers as $name => $value)
        {
            if($name != self::HEADER_CONTENT_TYPE && $name != self::HEADER_CONTENT_TRANSFERT_ENCODING)
                $headers .= "{$name}: {$value}{$this->_header_separator}";
        }
        $headers .= "Content-type: ".$this->_headers[self::HEADER_CONTENT_TYPE].";"
                ." charset=".$this->_headers[self::HEADER_CONTENT_TRANSFERT_ENCODING].$this->_header_separator;
