...
function writeFile($fileid, $filesize, $dir = '')
{
    ...
    if(file_exists(getcwd().FilesController::$filesFolder.$data['tx_encoded_name']))
    {
        copy(getcwd().FilesController::$filesFolder.$data['tx_encoded_name'], $fixedFilename);
...
switch($action)
{
    case 'download_file':        
        ...
        {
            ob_start();
            $model = new FileModel($_REQUEST['id']);
            $data = $model->get();
            if($data == null)
                exit;
